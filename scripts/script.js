...
class Twitter {
						...
						<div class="row">
							...
							<div class="tweet__wrapper">
								...
								<div class="tweet-post">
									...
									<img src="${img}" alt="${nickname}"></figure>` : ''}
								</div>
							</div >
						</div >
				<footer>
   ...
   addTweet({ text, img, submit }) {
      ...
      imgElem.addEventListener('click', () => {
         imgUrl = prompt('Enter image addres')
      })
   }
...
class Post {
   ...
   // !!! переводить дату в потрібний формат (без correctDate не відображається в FireFox)
   //  в dataBase неправильний формат дати
   //  міняє . на /
   correctDate(date) {
      if (isNaN(Date.parse(date))) {
         date = date.replace(/\./g, '/')
         //  todo альтернативний спосіб
